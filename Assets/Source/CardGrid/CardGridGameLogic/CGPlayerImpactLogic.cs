...
namespace CardGrid
{
    ...
    public partial class CardGridGame //PlayerImpactLogic
    {
        ...
        void CardMove()
        {
            ...
            if (_selectedCard.CardState.ScrObj.CardType == CT.Boots)
            {
                ButtonDrag();
            }
            else
        ...
        IEnumerator GetItemsFromField()
        {
            foreach (var item in _getItemsFomField)
            {
                ...
                yield return AddItem((item.ScrObj.CardType, item.StartQuantity));
            }
        ...
        IEnumerator UseItemOnFiled(CardGameObject dragCard, List<CardGameObject> highlightCards)
        {
            ...
            if (impactCardState.ScrObj.SoundEffect != null)
            {
                BattleAudioSource.clip = impactCardState.ScrObj.SoundEffect;
        ...
        IEnumerator CollectCards(CardState card)
        {
            yield break;
            ...
            // {
            //     if (card.ScrObj.ColorType == state.CollectColors[i].Item1)
            //     {
        ...
        */
        ...
        {
            bool needWait = false;
            var effect = impactCardState.ScrObj.Effect;
        ...
        float SpawnEffectOnCards(CardState[] cards)
        {
            ...
            foreach (var card in cards)
            {
                if (card.Quantity >= 0) //Target live?
                {
                    var go = GetEffect(card.ScrObj.Effect.name);

                    go.Color.SetColor(card.ScrObj.ColorType);
                    go.transform.position = BattleObjects.Field.GetCellSpacePosition(card.Position)
                                            + new Vector3(0, 0, -1f);
                    if(duration == 0)
        ...
        {
            var effect = cardState.ScrObj.Effect;
            if (effect)
            {
        ...
        void SetSwaying(CardGameObject cardGO)
        {
            if (_inputActive && _mySequence is not {active: true} && _swayingCard != cardGO)
            {
                ...
                if (cardState.ScrObj.Type == TypeCard.Crystal && cardState.Grid == CardGrid.Field)
                {
        ...
        {
            DebugSystem.DebugLog($"Item {itemCardState.ScrObj.Name}, impact on {fieldCardState.ScrObj.Name}" +
                                 $" X:{fieldCardState.Position.x} Y:{fieldCardState.Position.y}",
                DebugSystem.Type.PlayerInput);

            //TODO Push PickUp
            int[,] attackArray = GetImpactMap<ImpactMaps>(itemCardState.ScrObj.ImpactMap);

            return GetImpactedCards(itemCardState.ScrObj.Name, fieldCardState.Position, attackArray);
        }
        ...
        CardState[] GetImpactCards(CardState fieldCardState)
        {
            ...
            //TODO Push PickUp
            int[,] attackArray = GetImpactMap<ImpactMaps>(fieldCardState.ScrObj.ImpactMap);

            return GetImpactedCards(fieldCardState.ScrObj.Name, fieldCardState.Position, attackArray);
        }
    }
}